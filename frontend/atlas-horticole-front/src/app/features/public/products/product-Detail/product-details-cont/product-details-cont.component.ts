...
import { ProductDetails, RelatedProduct, TechnicalSpec } from '../../../../../core/models/product.details.model';


@Component({
  selector: 'app-product-details-cont',
  standalone: false,
  templateUrl: './product-details-cont.component.html',
  styleUrls: ['./product-details-cont.component.scss']
})
export class ProductDetailsContComponent implements OnInit {
  ...
  private loadProductDetails(productId: string): void {
    ...
    // Transformer le produit de base en ProductDetails avec données supplémentaires
    this.product = {
      ...baseProduct,
      technicalSpecs: this.getTechnicalSpecs(baseProduct),
      usageInstructions: this.getUsageInstructions(),
      relatedProducts: this.getRelatedProducts(baseProduct),
      documentationUrl: `/documents/${productId}.pdf`
    };
  }

  private getTechnicalSpecs(product: Product): TechnicalSpec[] {
    const specs: TechnicalSpec[] = [
      { name: 'Catégorie', value: product.category },
      { name: 'Disponibilité', value: product.inStock ? 'En stock' : 'Sur commande' }
    ];

    if (product.composition) {
      specs.push({ name: 'Composition', value: product.composition });
    }

    if (product.dosage) {
      specs.push({ name: 'Dosage recommandé', value: product.dosage });
    }

    // Ajouter des spécifications spécifiques selon la catégorie
    if (product.category === 'Biostimulants') {
      specs.push({ name: 'Type', value: 'Amendement organique' });
      specs.push({ name: 'Origine', value: '100% végétale' });
    } else if (product.category === 'Correcteurs de Carences') {
      specs.push({ name: 'Type', value: 'Chélaté' });
      specs.push({ name: 'Forme', value: 'Liquide/Solide' });
    }
  ...
  private getUsageInstructions(): string[] {
    return [
      ...
      'Consulter la fiche technique pour plus de détails',
      'Respecter les doses recommandées',
      'Conserver dans un endroit frais et sec'
    ];
  }

  private getRelatedProducts(product: Product): RelatedProduct[] {
  ...
  handleRequestQuote(): void {
    if (this.product) {
      ...
      console.log(`Demande de devis pour ${this.product.name}, quantité: ${this.quantity}`);
      // Naviguer vers la page de contact ou ouvrir un modal
  ...
  handleDownloadDocumentation(): void {
    if (this.product?.documentationUrl) {
      window.open(this.product.documentationUrl, '_blank');
    }
  }
